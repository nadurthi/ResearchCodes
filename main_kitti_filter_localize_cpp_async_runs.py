...
MTS={'fracstepstrack':[],'fracsteps2firsttrack':[],'fracstepstrack5mTruth':[],
      ...
      'rmsepos':[],'rmsedir':[],'N50stepstrack5mTruth':[],'N50stepstrack2.5mTruth':[]}
FPS=[]
plt.close("all")
...
for ff in simfiles:
    ...
    for i in range(len(simtKvec)):
        if np.all(nplinalg.norm(Xtpath[simtKvec[i:i+50],0:3]-mest[i:i+50,0:3],axis=1)<5):
            break
    MTS['N50stepstrack5mTruth'].append(i)
    
    for i in range(len(simtKvec)):
        if np.all(nplinalg.norm(Xtpath[simtKvec[i:i+50],0:3]-mest[i:i+50,0:3],axis=1)<2.5):
            break
    MTS['N50stepstrack2.5mTruth'].append(i)
    ...
    if rmsepos>10:
        print(simanger.data['k0'],rmsepos)
        print(ff)
...
##
methodstrings=['Nstepstrack','Nsteps2firsttrack','Nstepstrack5mTruth','N50stepstrack5mTruth','N50stepstrack2.5mTruth']
xpos1=np.arange(len(methodstrings),dtype=np.int)
...
ax0.set_xticklabels(['A','B','C','D','E'],fontsize=20)
...
MTS.hist(methodstrings,ax=ax3,xlabelsize=24,ylabelsize=24,bins=50,rwidth=0.9)
# ax3.set_xticklabels(methodstrings)
ax3.set_xlabel("RMSE (m)",fontsize=20)
...
# ax3.boxplot(MTS[methodstrings],positions = xpos1,boxprops={'linewidth':2},whiskerprops={'linewidth':2})
MTS.hist(methodstrings,ax=ax4,xlabelsize=24,ylabelsize=24,bins=50,rwidth=0.9)
# ax3.set_xticklabels(methodstrings)
ax4.set_xlabel("RMSE (m)",fontsize=20)
...
ax44.hist(FPSgg,bins=np.linspace(0,1,50),rwidth=0.9)
