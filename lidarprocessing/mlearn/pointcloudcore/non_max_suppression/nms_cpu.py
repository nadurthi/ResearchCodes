...
from lidarprocessing.mlearn.pytorchutils.buildtools.pybind11_build import load_pb11
from lidarprocessing.mlearn.pytorchutils.find import find_cuda_device_arch
import numba
import numpy as np

try:
    from lidarprocessing.mlearn.pointcloudcore.non_max_suppression.nms import (
        non_max_suppression_cpu, rotate_non_max_suppression_cpu)
except:
    ff=str(__file__)
    current_dir = Path(ff ).resolve().parents[0]
 
    
    load_pb11(["../cc/nms/nms_kernel.cu.cc", "../cc/nms/nms.cc"],current_dir / "nms.so",current_dir,cuda=True)
    from lidarprocessing.mlearn.pointcloudcore.non_max_suppression.nms import (
        non_max_suppression_cpu, rotate_non_max_suppression_cpu)
