...
import modelBuilders
import train_helpers
from dataset import merge_second_batch


from pytorchutils.eval import get_coco_eval_result, get_official_eval_result
from pytorchutils.progress_bar import ProgressBar


from pointcloudcore import VoxelGenerator
from pointcloudcore import region_similarity

from pytorchutils.losses import _get_pos_neg_loss

from pytorchutils.datamanipulators import _flat_nested_json_dict,flat_nested_json_dict,create_folder
...
with open("configs/car.config",'r') as F:
    config=json.load(F)

input_cfg = config['train_input_reader']
eval_input_cfg = config['eval_input_reader']
model_cfg = config['second']
...
voxel_generator = VoxelGenerator(
        voxel_size=list(config['voxel_generator']['voxel_size']),
        point_cloud_range=list(config['voxel_generator']['point_cloud_range']),
        max_num_points=config['voxel_generator']['max_number_of_points_per_voxel'],
        max_voxels=20000)
...
dataset = modelBuilder.input_reader_builder.build(
    input_cfg,
    model_cfg,
    training=True,
    voxel_generator=voxel_generator,
    target_assigner=target_assigner)
eval_dataset = modelBuilder.input_reader_builder(
    eval_input_cfg,
    model_cfg,
    training=False,
    voxel_generator=voxel_generator,
