...
def get_distance_transform(pts, label):
    ...
    if len(pts) > 0:
        ...
        # This is important since we divide it by 255 while normalizing the
        # inputs.
        return dt * 255
...
def encode_extreme_points(mask, use_gaussian):
    ...
    if use_gaussian:
        distance_transform_extreme_pts = norm.pdf(
            distance_transform_extreme_pts, loc=0, scale=10) * 25
    else:
