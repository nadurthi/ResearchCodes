...
def resize_image(img, out_size, bilinear):
    if bilinear:
        ...
        img = tf.image.resize_nearest_neighbor(
            tf.expand_dims(img, 0), out_size)
        img = tf.squeeze(img, 0)
    return img


def resize_coords(tensor, out_size, original_size):
    y_ratio = tf.ones([tf.shape(tensor)[0], 1]) * \
        tf.cast(out_size[0] / original_size[0], tf.float32)
    x_ratio = tf.ones([tf.shape(tensor)[0], 1]) * \
        tf.cast(out_size[1] / original_size[1], tf.float32)
...
def flip_coords_horizontal(tensor, original_width):
    ...
    return tf.stack([tensor[..., 0], original_width_f -
                     tensor[..., 3], tensor[..., 2], original_width_f -
                     tensor[..., 1]], axis=-
                    1)
    ...
    for idx in range(len(images)):
        ...
        if len(pts[0]) > 0:
            for pt in pts_zipped:
                if r < pt[0] < img.shape[0] - \
                        r and r < pt[1] < img.shape[1] - r:
