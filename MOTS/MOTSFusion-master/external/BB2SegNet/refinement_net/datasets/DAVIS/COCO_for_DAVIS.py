...
class DAVISLucidDataset(MapillaryLikeInstanceDataset):
    def __init__(self, config, subset):
        ...
        annotation_file = "/fastwork/" + \
            username() + "/mywork/data/coco/annotations/instances_train2014.json"
...
# Two things:
    ...
    def build_filename_to_coco_anns_dict(self, annotation_file):
        ...
        self.filename_to_coco_anns = {
            self.data_dir + img['file_name']: [] for img in imgs}
        ...
        self.filename_to_coco_anns = {
            f: anns for f,
            anns in self.filename_to_coco_anns.items() if len(anns) > 0}
    ...
    def load_ann(img, img_filename, annotation_filename):
        ...
        if ann['area'] > 1 and isinstance(ann['segmentation'], list):
            ...
            valid_segs = [np.asarray(p).reshape(-1, 2)
                          for p in segs if len(p) >= 6]
            if len(valid_segs) < len(segs):
                print("Image {} has invalid polygons!".format(img_filename))
            output_ann = np.asarray(self.segmentation_to_mask(
                valid_segs, img_h, img_w), dtype='uint8')[..., np.newaxis]  # Should be 1s and 0s
        else:
    ...
    def load_annotation(self, img, img_filename, annotation_filename):
        ...
        ann, = tf.py_func(
            load_ann, [
                img, img_filename, annotation_filename], [
                tf.uint8])
        # print(ann)
