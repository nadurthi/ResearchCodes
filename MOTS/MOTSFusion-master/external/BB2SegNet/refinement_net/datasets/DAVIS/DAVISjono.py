...
DAVIS_DEFAULT_IMAGES_PATH = "/work2/" + \
    username() + "/data/DAVIS/train-val/val/"
DAVIS_DEFAULT_GT_PATH = "/work2/" + username() + "/data/DAVIS/train-val/val-gt/"
DAVIS_DEFAULT_PROPOSALS_PATH = "/home/" + \
    username() + "/vision/maskrcnn_tensorpack/train_log/thesis1/notrain-val/"
...
class DAVISjonoDataset(FileListDataset):
    def __init__(self, config, subset, name="davisjono"):
        self.image_dir = config.string(
            "DAVIS_images_dir", DAVIS_DEFAULT_IMAGES_PATH)
        self.gt_dir = config.string("DAVIS_gt_dir", DAVIS_DEFAULT_GT_PATH)
        self.proposals_dir = config.string(
            "DAVIS_proposals_dir", DAVIS_DEFAULT_PROPOSALS_PATH)
    ...
    def load_annotation(self, img, img_filename, annotation_filename):
        ...
        def _load_ann(img_, ann_filename):
            ...
            bbox_ = self._tracking_gt[seq][id_][t].bbox_x0y0x1y1.astype("float32")[
                [1, 0, 3, 2]]
            return label_, bbox_
        label, bbox = tf.py_func(
            _load_ann, [
                img, annotation_filename], [
                tf.uint8, tf.float32])
