...
def resize_fixed_size(tensors, size):
    ...
    def resize_(key_, resizing_):
        if key_ in tensors:
            ...
            if resizing_ == Resizing_.Bilinear:
                ...
                resized = resize_coords(
                    val, size, tf.shape(tensors[DataKeys.IMAGES])[:2])
    ...
    keys_to_resize_nn = [
        ...
        DataKeys.RAW_SEGMENTATION_LABELS,
...
def resize_random_scale_with_min_size(
        tensors,
        min_size,
        min_scale=0.7,
        max_scale=1.3):
    ...
    min_scale_factor = tf.cast(min_size, tf.float32) / \
        tf.cast(shorter_side, tf.float32)
...
def resize_laser_to_fixed_size(laser, size):
    ...
    laser = tf.py_func(
        ...
        [laser],
        tf.float32,
        name="resize_laser_to_fixed_size")
    laser.set_shape(size + [1])
    return laser


def resize_laser_to_fixed_size_np(laser, size):
    # here we assume, that 1 is used for foreground, -1 for background and 0
    # for "no reading"
