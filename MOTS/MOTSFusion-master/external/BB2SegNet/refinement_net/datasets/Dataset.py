...
class AbstractDataset(ABC):
    def __init__(self, config, subset, num_classes):
        ...
        self.use_unsigned_distance_transform_guidance = config.bool(
            "use_unsigned_distance_transform_guidance", False)
        self.use_signed_distance_transform_guidance = config.bool(
            "use_signed_distance_transform_guidance", False)
    ...
    def load_raw_example(self, img_filename, label_filename=None, *args):
        ...
        label_tensors = self.load_annotation(
            img_tensors[DataKeys.IMAGES], img_filename, label_filename)
    ...
    def augment_example_before_resize(self, tensors):
        augmentors_str = self.config.string_list(
            "augmentors_" + self.subset, [])
    ...
    def assemble_example(self, tensors):
        ...
        if self.use_bbox_guidance and DataKeys.BBOX_GUIDANCE in tensors:
            ...
            bbox_guidance = tf.cast(
                tensors[DataKeys.BBOX_GUIDANCE], tf.float32)
        ...
        if self.use_laser_guidance and DataKeys.LASER_GUIDANCE in tensors:
            ...
            laser_guidance = tf.cast(
                tensors[DataKeys.LASER_GUIDANCE], tf.float32)
    ...
    def create_summaries(self, data):
        if DataKeys.IMAGES in data:
            self.summaries.append(tf.summary.image(
                self.subset + "data/images", unnormalize(data[DataKeys.IMAGES])))
...
class FileListDataset(AbstractDataset):
    ...
    def _load_inputfile_lists(self):
        ...
        if self.subset == "train":
            ...
            shuffle(zipped)
            self.inputfile_lists = tuple(
                [x[idx] for x in zipped] for idx in range(len(self.inputfile_lists)))
        assert len(self.inputfile_lists) > 0
        for l in self.inputfile_lists:
            assert len(l) > 0
        # make sure all lists have the same length
        assert all([len(l) == len(self.inputfile_lists[0])
                    for l in self.inputfile_lists])
    ...
    def create_input_tensors_dict(self, batch_size):
        ...
        if batch_size > 1:
            ...
            # like this we are able to retain the batch size in the shape
            # information
            tfdata = tfdata.map(
                lambda x: {
                    k: tf.expand_dims(
