        ...
        with tf.name_scope('gamma_augmentor'):
            ...
            if factor is None:
                factor = tf.random_uniform(
                    shape=[],
                    minval=self.gamma_range[0],
                    maxval=self.gamma_range[1],
                    dtype=tf.float32)
            gamma = tf.log(0.5 + 1 / math.sqrt(2) * factor) / \
                tf.log(0.5 - 1 / math.sqrt(2) * factor)
    ...
    def apply_after_resize(self, tensors, doit=None):
        with tf.name_scope("flip_augmentor"):
            ...
            def maybe_flip(key_, image_flip):
                if key_ in tensors:
                    ...
                    if image_flip:
                        ...
                        flipped = flip_coords_horizontal(
                            val, tf.shape(tensors[DataKeys.IMAGES])[1])
            ...
            keys_to_flip = [
                ...
                DataKeys.BBOX_GUIDANCE,
                DataKeys.SEGMENTATION_LABELS_ORIGINAL_SIZE,
                DataKeys.LASER_GUIDANCE,
                DataKeys.SEGMENTATION_MASK]
...
class BBoxJitterAugmentor(Augmentor):
    ...
    def apply_before_resize(self, tensors):
        if DataKeys.BBOXES_y0x0y1x1 in tensors:
            y0, x0, y1, x1 = tf.unstack(
                tf.cast(tensors[DataKeys.BBOXES_y0x0y1x1], tf.float32))
            g = tf.random_normal((4,))
            # avoid outliers by clipping. Otherwise the resulting bounding box
            # might even become empty
