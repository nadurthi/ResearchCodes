...
class COCOInstanceDataset(COCODataset):
    ...
    def build_filename_to_anns_dict(self):
        for ann in self.anns:
            ...
            if file_name in self.filename_to_anns:
                print(
                    "Ignoring instance as an instance with the same id exists in filename_to_anns.",
                    log.v1)
            else:
    ...
    def load_annotation(self, img, img_filename, annotation_filename):
        label, raw_label = tf.py_func(
            self._get_mask, [img_filename], [
                tf.uint8, tf.uint8], name="load_mask")
        label.set_shape((None, None, 1))
        raw_label.set_shape((None, None, 1))
        return {
            DataKeys.SEGMENTATION_LABELS: label,
            DataKeys.RAW_SEGMENTATION_LABELS: raw_label,
