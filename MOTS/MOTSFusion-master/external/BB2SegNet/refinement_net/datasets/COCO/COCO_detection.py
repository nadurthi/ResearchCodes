...
class CocoDetectionDataset(FileListDataset):
    ...
    def build_filename_to_coco_anns_dict(self, annotation_file):
        ...
        self.filename_to_img_ids = {
            img['file_name']: img['id'] for img in imgs}
        ...
        if self.subset is not 'val':
            ...
            self.filename_to_coco_anns = {
                f: anns for f, anns in self.filename_to_coco_anns.items() if len(anns) > 0}
    ...
    def get_data_arrays_for_file(self, img_filename, img_h, img_w):
        ...
        # TODO this will only work for batch size 1 since we arent filling up
        # to N_MAX_DETECTIONS, but less memory needed
        masks_list = []
        ...
        for idx, ann in enumerate(anns_for_img):
            ...
            ids[idx] = idx + 1
            # TODO double check correctness of these
            classes[idx] = self.cat_to_class[ann["category_id"]]
            ...
            if self.add_masks:
                if ann['area'] > 1 and isinstance(ann['segmentation'], list):
                    ...
                    valid_segs = [numpy.asarray(p).reshape(-1, 2)
                                  for p in segs if len(p) >= 6]
                    if len(valid_segs) < len(segs):
                        print(
                            "Image {} has invalid polygons!".format(img_filename))
                    masks_list.append(
                        numpy.asarray(
                            segmentation_to_mask(
                                ...
                                img_h,
                                img_w),
                            dtype='uint8'))  # Should be 1s and 0s
                else:
                    masks_list.append(
                        numpy.zeros(
                            (img_h, img_w), dtype="uint8"))

        if self.add_masks and len(masks_list) > 0:
            # format HWC, where C the different annotations, for resizing
            masks = numpy.stack(masks_list, axis=2)
        else:
    ...
    def load_annotation(self, img, img_filename, annotation_filename):
        ...
        bboxes, ids, classes, is_crowd, mask = tf.py_func(
            self.get_data_arrays_for_file, [
                img_filename, img_shape[0], img_shape[1]], [
                tf.float32, tf.int32, tf.int32, tf.int32, tf.uint8], name="get_data_arrays_for_file")
        ...
        if self.add_masks:
            # TODO shape is actually: img_shape[0], img_shape[1],
            # N_MAX_DETECTIONS but would have to use max/min img size -
            # problems with flip AND store dummy seg masks
    ...
    def prepare_saving_epoch_measures(self, epoch):
        ...
        self.det_file_path = model_folder + model_name + \
            "-" + str(epoch) + "-detections.json"
