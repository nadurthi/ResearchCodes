...
class COCODataset(FileListDataset):
    def __init__(self, config, subset, num_classes, name=NAME):
        ...
        if subset == "train":
            ...
            self.filter_crowd_images = config.bool(
                "filter_crowd_images", False)
        ...
        # Use the minival split as done in
        ...
        self.restricted_image_category_list = config.string_list(
            "restricted_image_category_list", [])
        if len(self.restricted_image_category_list) == 0:
            self.restricted_image_category_list = None
        self.restricted_annotations_category_list = config.string_list(
        ...
        self.exclude_image_category_list = config.string_list(
            "exclude_image_category_list", [])
        if len(self.exclude_image_category_list) == 0:
            self.exclude_image_category_list = None
        self.exclude_annotations_category_list = config.string_list(
            "exclude_annotations_category_list", [])
        if len(self.exclude_annotations_category_list) == 0:
            self.exclude_annotations_category_list = None

        # either both of them or none should be specified for now to avoid
    ...
    def filter_anns(self):
        ...
        if self.min_box_size != -1.0:
            self.filename_to_anns = {
                f: [
                    ann for ann in anns if ann["bbox"][2] >= self.min_box_size and ann["bbox"][3] >= self.min_box_size] for f,
                anns in self.filename_to_anns.items()}
        ...
        if self.restricted_image_category_list is not None:
            print(
                "filtering images to contain categories",
            ...
            for cat in self.restricted_image_category_list:
                n_imgs_for_cat = sum([1 for anns in self.filename_to_anns.values() if any(
                    [self.label_map[ann["category_id"] - 1]["name"] == cat for ann in anns])])
                print(
                    "number of images containing",
                    cat,
                    ":",
                    n_imgs_for_cat,
                    file=log.v5)
        ...
        if self.restricted_annotations_category_list is not None:
            print(
                "filtering annotations to categories",
            ...
            print("Excluding annotations for object categories",
                  self.exclude_annotations_category_list, file=log.v1)
            self.filename_to_anns = {f: [ann for ann in anns if self.label_map[ann["category_id"] - 1]["name"]
        ...
        self.filename_to_anns = {
            f: anns for f,
            anns in self.filename_to_anns.items() if len(anns) > 0}
    ...
    def load_image(self, img_filename):
        ...
        img_dir = tf.cond(
            tf.equal(
                tf.string_split(
                    [path], '_').values[1], tf.constant("train2014")), lambda: '%s/%s/' %
            (self.data_dir, "train2014"), lambda: '%s/%s/' %
    ...
    def read_inputfile_lists(self):
        ...
        # Filtering the image file names since some of them do not have
        # annotations.
        imgs = [img_dir + fn for fn in self.filename_to_anns.keys()]
        img_ids = [anns[0]["image_id"]
                   for fn, anns in self.filename_to_anns.items()]
        return imgs, imgs
