...
class FeedDataset(AbstractDataset):
        ...
        for key in self._data_keys_to_use:
            ...
            key_placeholders = [
                tf.placeholder(
                    ...
                    shape,
                    name=key +
                    "_placeholder_{}".format(idx)) for idx in range(
                    self._batch_size)]
    ...
    def create_input_tensors_dict(self, batch_size):
        examples = [self._create_input_tensors_dict_single_example(
            idx_in_minibatch=idx) for idx in range(batch_size)]
        keys = examples[0].keys()
        data = {}
        for k in keys:
            if batch_size > 1:
                # raw images don't work with batch_size > 1 since they have
                # different sizes
                if k == DataKeys.RAW_IMAGES:
                    continue
                data[k] = tf.stack([example[k]
