...
class DAVISFewShotSegmentationDataset(FewShotSegmentationDataset):
    ...
    def load_proposals(self, proposals_dir):
        ...
        print("pre-prop-load")
        _load_proposals = partial(
            load_proposal_for_tag,
    ...
    def _read_video_tags(self):
        ...
        print(
            curr_run_num,
            num_per,
            curr_run_num * num_per,
            (curr_run_num + 1) * num_per)
        sub_video_dirnames = video_dirnames[curr_run_num * \
            num_per: (curr_run_num + 1) * num_per]
    ...
    def _load_video_data(self, idx):
        ...
        with Pool(8) as pool:
            data_per_object = [
                x for x in pool.starmap(
                    _load_obj_data,
                    vid_proposals.items()) if x is not None]
