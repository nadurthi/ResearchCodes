...
def get_layer_class(layer_class):
    ...
    constructors = [
        l for l in Layer.__subclasses__() if l.__name__ == layer_class]
...
class NetworkTower:
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tower_setup.reuse_variables), \
            ...
            for name, layer_def in network_def.items():
                layer = self._create_layer(
                    name, layer_def, tower_setup, input_tensors_dict, dataset)
    ...
    def _create_layer(
        ...
        if "from" in layer_def:
            inputs = sum(
                [self.layers[x].outputs for x in layer_def["from"]], [])
            del layer_def["from"]
        else:
            inputs = [input_tensors_dict["inputs"]]
        if "concat" in layer_def:
            concat = sum(
                [self.layers[x].outputs for x in layer_def["concat"]], [])
