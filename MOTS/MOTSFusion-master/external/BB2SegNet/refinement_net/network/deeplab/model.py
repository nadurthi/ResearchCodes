...
def predict_labels_multi_scale(images,
        ...
        for output in sorted(outputs_to_scales_to_logits):
            ...
            if add_flipped_images:
                ...
                logits_reversed = tf.image.resize_bilinear(
                    tf.reverse_v2(
                        scales_to_logits_reversed[_MERGED_LOGITS_SCOPE],
                        [2]),
                    tf.shape(images)[
                        1:3],
                    align_corners=True)
    ...
    for output in sorted(outputs_to_predictions):
        ...
        # Compute average prediction across different scales and flipped
        # images.
...
def multi_scale_logits(images,
    ...
    if model_options.model_variant == 'mobilenet_v2':
        if (model_options.atrous_rates is not None or
            ...
            # Output a warning and users should make sure if the setting is
            # desired.
                ...
                if model_options.add_image_level_feature:
                    ...
                    if is_training:
                        pool_height = scale_dimension(
                            model_options.crop_size[0], 1. / model_options.output_stride)
                        pool_width = scale_dimension(
                            model_options.crop_size[1], 1. / model_options.output_stride)
                        image_feature = slim.avg_pool2d(
                            features, [
                                pool_height, pool_width], [
                                pool_height, pool_width], padding='VALID')
                    else:
                        pool_height = tf.shape(features)[1]
                        pool_width = tf.shape(features)[2]
                        image_feature = tf.reduce_mean(features, axis=[1, 2])[
    ...
    if aspp_with_batch_norm or atrous_rates is None:
        if kernel_size != 1:
            raise ValueError(
                ...
                'using aspp_with_batch_norm. Gets %d.' %
                kernel_size)
        atrous_rates = [1]
