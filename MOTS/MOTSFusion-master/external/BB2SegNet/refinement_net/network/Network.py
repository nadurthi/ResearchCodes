...
class Network:
    def __init__(
        ...
        self.input_tensors_dict = dataset.create_input_tensors_dict(
            self.batch_size)
        self._towers = self._build_towers(
            config,
            dataset,
            is_trainnet,
            freeze_batchnorm,
        ...
        if is_trainnet:
            print(
                ...
                "{:,}".format(
                    self._towers[0].n_params),
                file=log.v1)
        self.tower_total_losses_with_regularizers = [
            t.total_loss_with_regularizer for t in self._towers]
        ...
        with tf.name_scope(self.name):
            for gpu_idx in range(n_gpus):
                if n_gpus == 1:
                    ...
                    input_tensors_dict_sliced = {
                        k: v[slice_start:slice_end] for k, v in self.input_tensors_dict.items()}
                ...
                tower_setup = TowerSetup(
                    ...
                    is_training=is_trainnet,
                    is_main_train_tower=is_trainnet and gpu_idx == 0,
                    freeze_batchnorm=freeze_batchnorm,
                    network_name=self.name)
                tower = NetworkTower(
                    config, tower_setup, input_tensors_dict_sliced, dataset)
                towers.append(tower)
        return towers
