...
def conv2d_transpose(x, W, strides=(1, 1), padding="SAME"):
    ...
    out_height = deconv_output_length(
        inputs_shape[1], W_shape[0], padding, strides[0])
    out_width = deconv_output_length(
        inputs_shape[2], W_shape[1], padding, strides[1])
    output_shape = (inputs_shape[0], out_height, out_width, W_shape[2])

    return tf.nn.conv2d_transpose(x, W, tf.stack(output_shape), strides=[
...
def bootstrapped_ce_loss(raw_ce, fraction, n_valid_pixels_per_im):
    ...
    loss_per_im = tf.map_fn(
        bootstrapped_ce_for_one_img, [
            raw_ce, ks], dtype=tf.float32)
...
def class_balanced_ce_loss(raw_ce, targets, n_classes):
    ...
    def class_balanced_ce_for_one_img(args):
        ...
        for cls in range(n_classes):
            ...
            cls_loss = tf.reduce_sum(tf.boolean_mask(
                ce, cls_mask)) / tf.cast(tf.maximum(n_cls, 1), tf.float32)
            cls_losses.append(cls_loss)
        return tf.add_n(cls_losses) / n_classes

    loss_per_im = tf.map_fn(
        class_balanced_ce_for_one_img, [
            raw_ce, targets], dtype=tf.float32)
    return tf.reduce_mean(loss_per_im)
