...
def accumulate_measures(
    measures_accumulator,
    *new_measures,
    exclude=[
        DET_BOXES,
        DET_PROBS,
...
def compute_measures_for_binary_segmentation_tf(predictions, targets):
    def f(ps, ts):
        ...
        meas = [np.cast[np.float32](meas[IOU]), np.cast[np.float32](
            meas[RECALL]), np.cast[np.float32](meas[PRECISION])]
        return meas
    res = tf.py_func(f, [predictions, targets], [
...
def compute_measures_for_binary_segmentation_summed(predictions, targets):
    res = [
        compute_measures_for_binary_segmentation_single_image(
            p, t) for p, t in zip(
            predictions, targets)]
    accum = res[0]
    for r in res[1:]:
