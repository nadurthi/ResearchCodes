...
class Engine:
    def __init__(self, config, session=None):
        ...
        # need_train = True  # TODO should be self.task != "eval", but right
        # now testnet needs to reuse variables from train
        need_train = config.bool("need_train", True)
        if need_train:
            self.train_data = load_dataset(
                config, "train", self.session, self.dataset)
        ...
        if need_val:
            self.valid_data = load_dataset(
                config, "val", self.session, self.dataset)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            timer = Timer()
            train_measures = self.run_epoch(
                self.trainer.train_step,
                self.train_data,
                epoch,
                is_train_run=True)
            if self.valid_data is not None:
                valid_measures = self.run_epoch(
    ...
    def run_epoch(step_fn, data, epoch, is_train_run):
        ...
        if not is_train_run and hasattr(
                data, "finalize_saving_epoch_measures"):
