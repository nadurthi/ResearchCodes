...
class Trainer:
    def __init__(
        ...
        self.summary_writer, self.summary_op_train, self.summary_op_test = self.init_summaries(
            config, grad_norm)
    ...
    def create_optimizer(self, config):
        ...
        if self.opt_str == "sgd_nesterov":
            return tf.train.MomentumOptimizer(
                self.lr_var, momentum, use_nesterov=True), None
    ...
    def init_summaries(self, config, grad_norm=None):
        ...
        if config.bool("write_summaries", True):
            ...
            if self.test_network is not None and len(
                    self.test_network.summaries) > 0:
    ...
    def _step(
        ...
        if len(extraction_keys) > 0:
            ops[Extractions.EXTRACTIONS] = [{k: [v] for k, v in extractions.items(
            ) if k in extraction_keys} for extractions in network.tower_extractions]
        ...
        res[Measures.MEASURES] = accumulate_measures(
            {}, *res[Measures.MEASURES])
        if len(extraction_keys) > 0:
            res[Extractions.EXTRACTIONS] = accumulate_extractions(
