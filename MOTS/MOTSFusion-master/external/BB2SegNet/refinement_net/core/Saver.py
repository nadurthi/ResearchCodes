...
class Saver:
        ...
        if fn is not None:
            ...
            if self.load_init_savers is None:
                self.load_init_savers = [
                    self._create_load_init_saver(x) for x in self.load_init]
            assert len(self.load_init) == len(self.load_init_savers)
            for fn, load_init_saver in zip(
                    self.load_init, self.load_init_savers):
                if fn.endswith(".pickle"):
                    print(
                        "trying to initialize model from wider-or-deeper mxnet model", fn)
    ...
    def _create_load_init_saver(self, filename):
        ...
        if filename == "" or filename.endswith(
                ".pickle") or filename.startswith("DeepLabRGB:"):
            return None

        vars_and_shapes_file = [x for x in list_variables(
            filename) if x[0] != "global_step"]
        ...
        if len(vars_missing_in_graph) > 0:
            print(
                "the following variables will not be initialized since they are not present in the initialization model", [
        ...
        if len(vars_missing_in_file) > 0:
            print(
                ...
                vars_missing_in_file,
                file=log.v1)

        vars_shape_mismatch = [x for x in vars_intersection if x.shape.as_list(
        ) != vars_to_shapes_file[x.name[:-2]]]
        if len(vars_shape_mismatch) > 0:
            print(
                "the following variables will not be loaded from the file since the shapes in the graph and in the file "
                "don't match:", [
                    (x.name, x.shape) for x in vars_shape_mismatch if "Adam" not in x.name], file=log.v1)
            vars_intersection = [
    ...
    def _initialize_deep_lab_rgb_weights(self, fn):
        ...
        var_w = [x for x in vars_ if x.name ==
...
def load_wider_or_deeper_mxnet_model(model_path, session):
    ...
    # use a placeholders to avoid memory issues when putting the weights as
    ...
    for idx, (k, v) in enumerate(mxnet_dict.items()):
        ...
        if var.get_shape() == val.shape:
            ...
            print(
                "warning, sizes for",
                k,
                "do not match, initializing matching part assuming the first 3 dimensions are RGB",
                file=log.v1)
            ...
            print("skipping", k, "since the shapes do not match:",
                  var.get_shape(), "and", val.shape, file=log.v1)
