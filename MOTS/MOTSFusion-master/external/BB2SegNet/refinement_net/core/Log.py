# this file is from RETURNN
# (https://github.com/rwth-i6/returnn/blob/master/Log.py)
import logging
import os
import sys
...
class Stream:
    ...
    def flush(self):
        with self.lock:
            ...
            # In Python 2.7, it incorrectly does. See:
...
class Log(object):
    def initialize(self, logs=[], verbosity=[], formatter=[]):
        ...
        for l in self.v:
            # Reset handler list, in case we have initialized some earlier
            # (e.g. multiple log.initialize() calls).
            l.handlers = []
        ...
        for i in range(len(logs)):
            ...
            f = fmt['default'] if i >= len(
                formatter) or formatter[i] not in fmt else fmt[formatter[i]]
            if t == 'stdout':
