...
class EvalPascalRecursive(Engine):
    ...
    def eval(self):
        for round in range(self.recursive_rounds):
            ...
            while n_examples_processed < n_examples_per_epoch:
                ...
                # add tag to the measures so that it is printed (postprocessing
                # for davis)
                measures_str = self.add_tag(
                    measures_str, round, res[Extractions.EXTRACTIONS])
                accumulate_measures(measures_accumulated, measures)
                elapsed = timer.elapsed()
                self.valid_data.use_segmentation_mask(res)
                self.update_iou(
                    measures, res[Extractions.EXTRACTIONS], round + 1)
                print(
                    "{:>5}".format(n_examples_processed),
                    '/',
                    ...
                    elapsed,
                    file=log.v5)
            measures_averaged = compute_measures_average(
                measures_accumulated, for_final_result=True)
            print(
                "Click ",
                round,
        ...
        print("Avg clicks for", self.iou_threshold * 100, " % IOU is ",
              np.average(list(self.avg_clicks.values())), file=log.v1)
    ...
    def add_tag(self, measures, round, extractions):
        ...
        measures += "  {" + img_filename + "}  " + \
            "{clicks: " + str(round) + "}"
        return measures

    def update_iou(self, measures, extractions, click):
        img_filename = extract(extractions, DataKeys.IMAGE_FILENAMES)
        measures_avg = compute_measures_average(
            measures, for_final_result=False)
        iou = float(measures_avg[Measures.IOU])

        if (iou >= self.iou_threshold or click ==
                self.recursive_rounds) and img_filename not in self.avg_clicks:
            self.avg_clicks[img_filename] = click

    def postprocess(self, res):
        result_measures = res[Measures.MEASURES]
        img_filename = extract(
            res[Extractions.EXTRACTIONS], DataKeys.IMAGE_FILENAMES)
        ...
        if prev_mask is not None:
            ...
            if self.locality:
                prediction = np.where(
                    dt > self.locality_threshold, prev_mask, prediction)
        ...
        res[Extractions.EXTRACTIONS][Extractions.SEGMENTATION_MASK_INPUT_SIZE][0] = [
            prediction]
