...
def get_fn_with_worst_iou(seq):
    ...
    for file in files:
        ...
        gt = imread(gt_file)
        gt = gt / 255
        measure = compute_measures_for_binary_segmentation_single_image(
            img, gt)
        if measure is None:
            print(fn_file, gt_file, measure)
        if result_measure is None or measure[IOU] < result_measure[IOU]:
            result_measure = measure
            result_fn = DAVIS_PATH + "/JPEGImages/480p/" + \
                seq_name + "/" + fname.replace(".png", ".jpg")
            result_gt = gt_file
...
class OSVOSWorst(FileListDataset):
    ...
    def read_inputfile_lists(self):
        ...
        seqs = [
            os.path.join(
                pre_computed,
                f) for f in os.listdir(pre_computed) if os.path.isdir(
                os.path.join(
                    pre_computed,
                    f))]
        for seq in seqs:
