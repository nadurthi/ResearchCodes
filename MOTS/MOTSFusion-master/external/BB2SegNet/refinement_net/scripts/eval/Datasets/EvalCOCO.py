...
class EvalCOCODataset(COCOMaskedDiosDataset):
    def __init__(self, config, subset):
        ...
        self.randomly_sample_images = config.bool(
            "randomly_sample_images", False)
    ...
    def filter_anns(self):
        ...
        if self.restricted_image_category_list is not None:
            ...
            for cat in self.restricted_image_category_list:
                self.cat_to_filenames[cat] = [fn for fn, anns in self.filename_to_anns.items(
                ) if any([self.label_map[ann["category_id"] - 1]["name"] == cat for ann in anns])]
                print("number of images containing", cat, ":",
                      len(self.cat_to_filenames[cat]), file=log.v5)
        ...
        self.filename_to_anns = {
            f: [
                ann for ann in anns if ann["area"] >= filter_area *
                filter_area] for f,
