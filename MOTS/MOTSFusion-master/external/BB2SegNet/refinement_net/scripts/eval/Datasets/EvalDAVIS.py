...
class EvalDAVISContextClicks(EvalDAVISDataset):
    ...
    def create_summaries(self, data):
        ...
        if DataKeys.NEG_CLICKS in data:
            self.summaries.append(tf.summary.image(
                self.subset + "data/neg_clicks", tf.cast(data[DataKeys.NEG_CLICKS][:, :, :, 1:2], tf.float32)))
        if DataKeys.POS_CLICKS in data:
            self.summaries.append(tf.summary.image(
                self.subset + "data/pos_clicks", tf.cast(data[DataKeys.POS_CLICKS][:, :, :, 1:2], tf.float32)))
