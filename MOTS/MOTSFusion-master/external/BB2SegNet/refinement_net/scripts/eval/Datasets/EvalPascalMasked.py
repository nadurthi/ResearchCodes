...
def generate_click_for_correction(
        label,
        prediction,
        previous_clicks,
        void_label,
    ...
    if random_cluster:
        print(
            "Using mislabelled clusters sampled with probability corresponding to its size.",
            file=log.v1)
        largest_cluster = np.random.choice(
            cluster_counts, p=cluster_counts / np.sum(cluster_counts))
        ...
        largest_cluster = np.argmax(
            np.delete(
                np.bincount(
                    misclassified.flatten()),
                0,
                axis=0)) + 1

    dt = np.where(misclassified == largest_cluster, 1, 0)
    dt = misclassified
    # Set the border pixels of the image to 0, so that the click is centred on
    # the required mask.
    dt[[0, dt.shape[0] - 1], :] = 0
    dt[:, [0, dt.shape[1] - 1]] = 0
    ...
    for i in range(n_clicks):
        ...
        if np.max(dt) > 0:
            ...
            # sample from the list since there could be more that one such
            # points.
...
class EvalPascalMaskedDataset(PascalVOCMaskedDiosDataset):
    ...
    def dios_distance_transform(
        ...
        if img_filenames not in self.previous_epoch_data:
            ...
            if PREV_MASK not in self.previous_epoch_data[img_filenames]:
                print(
                    ...
                    img_filenames,
                    self.previous_epoch_data[img_filenames])
            prediction = decodeMask(
                self.previous_epoch_data[img_filenames][PREV_MASK])
        ...
        clicks = generate_click_for_correction(
            label,
            prediction,
        ...
        if np.any(label[np.where(u0[:, :, 0] == 0)]):
            print("Neg clicks on the object detected",
                  label[np.where(u0[:, :, 0] == 0)], file=log.v1)
        ...
        return u0, u1, np.array(neg_clicks).astype(
            np.int64), np.array(pos_clicks).astype(
            np.int64), num_clicks
    ...
    def use_segmentation_mask(self, res):
        ...
        if self.save_images:
            ...
            fn = main_folder + filename.split(".")[0].split("/")[-1] + "_" + str(
                filename.split(":")[-1]) + str(num_clicks) + ".png"
            img = unnormalize(inputs[:, :, :3])
            img = Util.get_masked_image(img, prediction)
            click_map = np.ones_like(inputs[:, :, 3:4])
            previous_clicks = [a for a in zip(
                *(val for val in prev_neg_clicks))]
            click_map[previous_clicks] = 0
            img = visualise_clicks(
                [img], np.expand_dims(
                    click_map, axis=0), 'r')

            click_map = np.ones_like(inputs[:, :, 3:4])
            previous_clicks = [a for a in zip(
                *(val for val in prev_pos_clicks))]
            click_map[previous_clicks] = 0
