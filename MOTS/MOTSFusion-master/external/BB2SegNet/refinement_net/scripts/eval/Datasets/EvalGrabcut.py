...
class EvalGrabcutDataset(GrabcutDataset):
    ...
    def postproc_example_before_assembly(self, tensors):
        return self.eval_pascal_dataset.postproc_example_before_assembly(
            tensors)

    def postproc_annotation(self, ann_filename, ann):
        mask = super().postproc_annotation(ann_filename, ann)
        return {
            DataKeys.SEGMENTATION_LABELS: mask,
            DataKeys.RAW_SEGMENTATION_LABELS: mask,
    ...
    def create_summaries(self, data):
        if DataKeys.IMAGES in data:
            ...
            if DataKeys.NEG_CLICKS in data:
                images = tf.py_func(visualise_clicks, [
                                    images, data[DataKeys.NEG_CLICKS][:, :, :, 0:1], "r"], tf.float32)
                self.summaries.append(tf.summary.image(
                    self.subset + "data/neg_clicks", tf.cast(data[DataKeys.NEG_CLICKS][:, :, :, 0:1], tf.float32)))
            if DataKeys.POS_CLICKS in data:
                images = tf.py_func(visualise_clicks, [
                                    images, data[DataKeys.POS_CLICKS][:, :, :, 0:1], "g"], tf.float32)
                self.summaries.append(tf.summary.image(
                    self.subset + "data/pos_clicks", tf.cast(data[DataKeys.POS_CLICKS][:, :, :, 0:1], tf.float32)))
    ...
    def use_segmentation_mask(self, res):
        ...
        if self.iterative_training:
            ...
            for id in range(batch_size):
                ...
                self.previous_epoch_data[filename][PREV_MASK] = encodeMask(
                    extractions[Extractions.SEGMENTATION_MASK_INPUT_SIZE][0][id])
        ...
        if self.save_images:
            ...
            fn = main_folder + \
                filename.split("/")[-1].split(".")[0] + "_" + str(num_clicks) + ".png"
