...
class FewShotSegmentationForwarder(Forwarder):
    def __init__(self, engine):
        ...
        self.print_per_object_stats = self.config.bool(
            "print_per_object_stats", True)

    def forward(self):
        video_ids = range(
            self.val_data.n_videos()) if len(
            self.video_ids) == 0 else self.video_ids
        for video_idx in video_ids:
            ...
            obj_ids = range(
                n_objects) if video_idx not in self.object_ids else self.object_ids[video_idx]
            for obj_idx in obj_ids:
                with Timer("object " + str(obj_idx) if self.print_per_object_stats else None):
                    ...
                    # reset weights and optimizer for next video/object (only
                    # if we actually do fine-tuning)
    ...
    def _forward_object(self, video_idx, obj_idx, tag):
        ...
        for i in range(n_partitions):
            ...
            end_step = min(
                start_step + forward_interval,
                self.n_finetune_steps)
    ...
    def _finetune(self, tag, start_step, end_step):
        for step in range(start_step, end_step):
            ...
            try:
                placeholders = self.train_data.get_placeholders(
    ...
    def _forward(self, step_num):
        ...
        # measures_string_to_print(compute_measures_average(measures,
        # for_final_result=True)), file=log.v4)
