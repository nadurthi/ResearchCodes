...
def calculate_association_similarities(
        detections_t,
        last_tracks,
        flow_tm1_t,
        tracker_options):
    ...
    if tracker_options["reid_weight"] != 0:
        ...
        association_similarities += tracker_options["reid_weight"] * \
            reid_similarities
    ...
    if tracker_options["bbox_center_weight"] != 0:
        centers_t = [v[0][0:2] +
                     (v[0][2:4] - v[0][0:2]) / 2 for v in detections_t]
        centers_tm1 = [v.box[0:2] +
                       (v.box[2:4] - v.box[0:2]) / 2 for v in last_tracks]
        box_dists = cdist(
            np.array(centers_t),
            np.array(centers_tm1),
            "euclidean")
        box_similarities = tracker_options["box_scale"] * \
            (tracker_options["box_offset"] - box_dists)
        association_similarities += tracker_options["bbox_center_weight"] * \
            box_similarities
    if tracker_options["bbox_iou_weight"] != 0:
        ...
        bbox_ious = np.array([[bbox_iou(box1, box2)
                               for box1 in bboxes_tm1_warped] for box2 in bboxes_t])
