...
def compute_point_imgs(config, sequence, calib_params):
    assert os.path.exists(config.dir(
        'flow_disp_savedir')), "Run flow and disp estimation (netdef) before computing pointcloud."
    assert os.path.exists(config.dir('orb_pose_savedir')) or os.path.exists(
        config.geo_pose_savedir), "Run pose estimation (geonet, orbslam) before computing pointcloud."
    ...
    if config.string('use_pose') == 'geonet':
        pose_list = pickle.load(
            open(
                config.dir('geo_pose_savedir') +
                sequence +
                '/out_poses.pkl',
                'rb'))
    else:
        pose_list = np.genfromtxt(
            config.dir('orb_pose_savedir') +
            sequence +
    ...
    disp_list = sorted(
        filter(
            lambda x: 'png.disp' in x,
            os.listdir(
                config.dir('flow_disp_savedir') +
                sequence +
                '/')))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        dest='config',
        type=str,
        default='./configs/config_default')
    parser.add_argument(
        '-compute_seg',
        help='compute segmentations',
        dest='compute_seg',
        type=bool,
        default=True)
    parser.add_argument(
        '-compute_flow_and_disp',
        help='compute flow and disp using netdef',
        dest='compute_flow_and_disp',
        type=bool,
        default=True)
    parser.add_argument(
        '-compute_poses',
        help='compute poses using ORB-SLAM',
        dest='compute_poses',
        type=bool,
        default=True)
    parser.add_argument(
    ...
    for sequence in list_sequences:
        ...
        calibration_params = CalibrationParameters(
            sequence_dir + sequence + '.txt')

        if args.compute_seg:
            print('computing segmentations..')
            detections = import_detections(config, sequence)
            compute_segmentations(refinement_net, sequence_dir, config.dir(
                'segmentations_savedir') + sequence + '/', detections)
            print('done.')

        if args.compute_flow_and_disp:
            print('computing flow and disp..')
            compute_flow_and_disp(
                config.dir('flow_disp_savedir') + sequence + '/',
                sequence_dir,
                config.dir('netdef_disp_modeldir'),
