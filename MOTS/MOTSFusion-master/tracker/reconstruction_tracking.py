...
def get_trust_regions(
        config,
        tracks,
        track_id,
        start_step=None,
        stop_step=None):
    track_covariances = tracks.get_track_attribute(
        track_id, 'transform_covariances')
    ...
    while step < stop_step:
        if compute_cov_norm(
                track_covariances[step]) < residual_thresh and step < stop_step - 1 and tracks.get_attribute(
                step - 1,
                track_id,
                'global_positions') is not None:
            trust_regions.append([step])
            center_point = get_center_point(
                config,
                tracks.get_attribute(
                    step - 1,
                    track_id,
                    'global_points'),
                tracks.get_detection(
                    step - 1,
                    track_id)['class'])
            transform_per_region.append([shift_transform(
                center_point, tracks.get_attribute(step, track_id, 'dynamic_transforms'))])
            center_point = transform(
                tracks.get_attribute(
                    step - 1,
                    track_id,
                    'global_positions'),
            ...
            while r < residual_thresh:
                ...
                transform_per_region[-1].append(shift_transform(
                    center_point, tracks.get_attribute(step + count, track_id, 'dynamic_transforms')))
                center_point = transform(
                    center_point, tracks.get_attribute(
                        step, track_id, 'dynamic_transforms'))
                # if r < min_r:
    ...
    return [
        region for region in trust_regions if len(region) > 1], [
        transform for transform in transform_per_region if len(transform) > 1]
...
def get_params(
        ...
        reference_id,
        step,
        steps_to_extrapolate,
        forward=True):
    if forward:
        trust_regions, transform_per_region = get_trust_regions(
            config, tracks, reference_id, stop_step=step)
    else:
        trust_regions, transform_per_region = get_trust_regions(
            config, tracks, reference_id, start_step=step)
    # if forward:
    ...
    if len(trust_regions):
        ...
        start_point = tracks.get_attribute(
            trust_regions[idx][idx], reference_id, 'global_positions')
        for _ in range(steps_to_interpolate):
            shifted_avg_transform = inv_shift_transform(
                start_point, avg_transform)
            if forward:
                start_point = transform(start_point, shifted_avg_transform)
            else:
                start_point = transform(
                    start_point, shifted_avg_transform, inverse=True)

        predict_points = [start_point]
        for _ in range(steps_to_extrapolate):
            shifted_avg_transform = inv_shift_transform(
                predict_points[-1], avg_transform)
            if forward:
                predict_points.append(
                    transform(predict_points[-1], shifted_avg_transform))
            else:
                predict_points.append(transform(
                    predict_points[-1], shifted_avg_transform, inverse=True))
        ...
        if predict_points[0] is not None and predict_points[1] is not None:
            if forward:
                ...
                avg_transform = tracks.get_attribute(
                    step, reference_id, 'dynamic_transforms')
            else:
                avg_transform = tracks.get_attribute(
                    step + 1, reference_id, 'dynamic_transforms')
...
def track_consistency(
    ...
    predict_points_ref, avg_transform_ref, trust_regions_ref = get_params(
        config, tracks, point_img_list, reference_track, start_step, steps_to_extrapolate)
    ...
    for proposal in track_proposals:
        ...
        predict_points_proposal, avg_transform_proposal, trust_regions_proposal = get_params(
            config, tracks, point_img_list, proposal, current_step, steps_to_extrapolate, forward=False)
        ...
        if not reference_track == proposal:
            for point_ref, point_proposal in zip(
                    predict_points_ref, predict_points_proposal):
                diff = np.asarray(point_ref) - np.asarray(point_proposal)
                diff = [diff[0], diff[2]]

                depth_covariance_ref = get_depth_covariance(
                    point_ref, pose_list[start_step + num_timesteps], calibration_params)
                depth_covariance_proposal = get_depth_covariance(
                    point_proposal, pose_list[current_step - num_timesteps], calibration_params)

                if config.bool('debug'):
                    print('depth_covariance_ref', depth_covariance_ref)
                    print(
                        "depth_covariance_proposal",
                        depth_covariance_proposal)

                    print('avg_transform_ref', avg_transform_ref)
                    print('avg_transform_proposal', avg_transform_proposal)
                    avg_transform = (
                        (avg_transform_ref + avg_transform_proposal) / 2)
                    print('avg_transform', avg_transform)

                # / np.log(np.abs(avg_transform[0]) + 2)
                diff[0] = diff[0] / np.log(depth_covariance_ref[0]
                                           [0] + depth_covariance_proposal[0][0] + np.e)
                # / np.log(np.abs(avg_transform[1]) + 2)
                diff[1] = diff[1] / np.log(depth_covariance_ref[2]
                                           [2] + depth_covariance_proposal[2][2] + np.e)
                #
                # print('diff[0]', diff[0])
                # print('diff[1]', diff[1])
        ...
        if config.bool('debug'):
            print('consistency score for ' + str(reference_track) +
                  ' and ' + str(proposal) + ': ' + str(consistency_score))
            print('..............')
        ...
        if consistency_score < thresh:
            ...
            overlapping_points = zip(
                predict_points_ref, predict_points_proposal)
            thresh = consistency_score

    if len(trust_regions_ref) and result_proposal_trust_region_len and steps_to_extrapolate > 1:
        prop_points = tracks.get_attribute(
            current_step, result_proposal_id, 'global_points')
        ref_points = tracks.get_attribute(
            start_step, reference_track, 'global_points')
        if prop_points is not None and ref_points is not None:
            evaluated_class = tracks.get_detection(
                start_step, reference_track)['class']
            bbox_start = get_center_point(config, ref_points, evaluated_class)
            bbox_current = get_center_point(
                config, prop_points, evaluated_class)
            fill_bbox_params = [
                bbox_start,
                bbox_current,
                warping_start,
...
def fill_merged_track(
    ...
    # here we want modal bboxes not amodal, hence take mask
    bbox_start = masktoBbox(
        tracks.get_mask(
            start_step,
            reference_id,
            decode=False))
    bbox_current = masktoBbox(
        tracks.get_mask(
            current_step,
            reference_id,
            decode=False))
    ...
    for timestep in range(num_steps):
        bboxes_all.append((bboxes_all[-1][0] + bbox_delta,
                           flowwarp_box(bboxes_all[-1][0],
                                        flow[start_step + timestep])))

    bboxes_all.pop(0)

    bboxes_ref, bboxes_proposal, bbox_start2, bbox_current2 = compute_bboxes(
        fill_bbox_params, start_step, current_step)
    ...
    current_point_box_distance = get_point_box_distance(
        compute_final_bbox(
            ...
            get_bbox_points(
                config,
                bbox_current2),
            point_img_list[current_step],
            pose_list[current_step],
            calibration_params),
        point_img_list[current_step],
        [
            bbox_current2[0],
    ...
    avg_point_box_distance = (
        start_point_box_distance + current_point_box_distance) / 2
    ...
    for index, bboxs in enumerate(bboxes_all):
        ...
        points = get_bbox_points(
            config,
            (np.asarray(
                bboxes_ref[index]) +
                np.asarray(
                bboxes_proposal[index])) /
            2)
        ...
        [x1,
         y1,
         x2,
         y2] = compute_final_bbox_flow(bbox,
                                       bbox_flow,
                                       points,
                                       point_img,
                                       pose_list[start_step + index],
                                       calibration_params)

        avg_predicted_position = (np.asarray(
            bboxes_ref[index]) + np.asarray(bboxes_proposal[index])) / 2
        avg_predicted_position = np.asarray(
            [avg_predicted_position[0], avg_predicted_position[2]])

        point_box_distance = get_point_box_distance(
            [x1, y1, x2, y2], point_img, avg_predicted_position)
        ...
        if point_box_distance < np.minimum(4, 1.8 * avg_point_box_distance):
            ...
            mask = segment([detection],
                           img_list[start_step + index + 1],
                           refinement_net)[0]
            tracks.add_to_track(
                start_step + index + 1,
                reference_id,
            ...
            if config.bool('debug'):
                avg_box = (
                    ...
                    np.asarray(
                        bboxes_proposal[index]))
                avg_box[3] = np.arctan((np.sin(bboxes_ref[index][3]) +
                                        np.sin(bboxes_proposal[index][3])) /
                                       (np.cos(bboxes_ref[index][3]) +
                                        np.cos(bboxes_proposal[index][3])))
                tracks.set_attribute(
                    start_step + index + 1,
                    reference_id,
                    'global_3D_bbox',
                    avg_box)
...
def extrapolate_terminated_track(
        config,
        tracks,
        flow_list,
        pose_list,
    ...
    if inverse:
        _, avg_transform_ref, trust_regions_ref = get_params(
            config, tracks, point_img_list, reference_id, start_step, 0, forward=False)
    else:
        _, avg_transform_ref, trust_regions_ref = get_params(
            config, tracks, point_img_list, reference_id, start_step, 0)
    ...
    while in_frame and point_box_distance < np.minimum(
            4, 1.8 * avg_point_box_distance) and not reached_eos:
        # update
        shifted_avg_transform = inv_shift_transform(
            bbox_ref[0:3], avg_transform_ref)
        if inverse:
            ...
            bbox_ref[0:3] = transform(
                bbox_ref[0:3], shifted_avg_transform, inverse=True)
        ...
        [x1, y1, x2, y2] = compute_final_bbox_flow(
            bbox, bbox_flow, points, point_img, pose_list[current_step], calibration_params)

        # check bbox
        predicted_position = np.asarray([bbox_ref[0], bbox_ref[2]])
        point_box_distance = get_point_box_distance(
            [x1, y1, x2, y2], point_img, predicted_position)
        ...
        if config.bool('debug'):
            ...
            print('in_frame', in_frame and point_box_distance <
                  np.minimum(4, 1.8 * avg_point_box_distance))

        if in_frame and point_box_distance < np.minimum(
                3, 1.8 * avg_point_box_distance):
            detection = {}
            detection['bbox'] = [x1, y1, x2, y2]
            detection['score'] = 1.0
            detection['class'] = evaluated_class
            ...
            count += 1
            if not inverse:
                in_frame = x1 > 20 and y1 > 10 and x2 < point_img.shape[1] - \
                    20 and y2 < point_img.shape[0] - 10
...
def merge_tracks(
    ...
    for step in range(tracks.timesteps - 1):
        ...
        for ref_id in tracks.get_active_tracks(step):
            ...
            # if is active and is not an already filled track (=no additional
            # information to mask and detection)
            if not tracks.is_active(
                    step + 1,
                    ref_id) and tracks.get_attribute(
                    step,
                    ref_id,
                    'global_positions') is not None:
                timesteps = range(
                    step + 1,
                    np.minimum(
                        step + config.int('merging_timesteps'),
                        tracks.timesteps - 1))
                for t in timesteps:
                    proposals = []
                    ...
                    if len(proposals):
                        merging_candidate, fill_bbox_params = track_consistency(
                            config, tracks, ref_id, proposals, step, t, point_img_list, img_list, pose_list, calibration_params)
                    else:
                        merging_candidate = -1
                        fill_bbox_params = None
                    ...
                    if merging_candidate != -1:
                        ...
                        if plausible:
                            ...
                            if ref_id not in merged_ids.keys():
                                merged_ids[ref_id] = [merging_candidate]
                            else:
                                merged_ids[ref_id].append(merging_candidate)

                            if config.str('mode') == 'MOTS' and t - \
                                    step > 1 and fill_bbox_params is not None:
                                fill_merged_track(
                                    config,
                                    tracks,
                                    fill_bbox_params,
...
def extrapolate_final_tracks(
    ...
    if config.str('mode') == 'MOTS':
        for ref_id in range(tracks.get_num_ids()):
            ...
            if any(track):
                locations = [loc for loc,
                             val in enumerate(track) if val is True]
                end_step = max(locations)
                start_step = min(locations)

                if 0 < end_step < tracks.timesteps - 1:
                    extrapolate_terminated_track(
                        config,
                        tracks,
                        flow_list,
                        pose_list,
