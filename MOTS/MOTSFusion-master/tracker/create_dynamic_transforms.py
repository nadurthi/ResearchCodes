...
def remove_outliers(object_points):
    if len(object_points) > 100:
        ...
        for points in [points_t0, points_t1]:
            ...
            mask = np.logical_or(
                [X_scores[i] > median_score for i in range(len(points))], mask)
...
def sparsify(object_points):
    if len(object_points) > 200:
        undersample = np.random.uniform(
            0, len(object_points), 200).astype(
            np.int)
    ...
    for step in range(tracks.timesteps - 1):
        for id in tracks.get_active_tracks(step):
            if id in tracks.get_active_tracks(step + 1):
                ...
                points, colors = get_points_from_masks(
                    mask_t0, mask_t1, point_imgs[step], point_imgs[step + 1], flow[step + 1], raw_imgs[step], raw_imgs[step + 1], calibration_params)
                ...
                if len(points) > 1:
                    ...
                    if not tracks.is_active(step - 1, id):
                        tracks.set_attribute(step, id, 'position_covariances', get_position_covariance(
                            points_processed[:, 0], calibration_params))
                        tracks.set_attribute(step, id, 'global_positions', np.mean(
                            points_processed[:, 0], axis=0))
                        tracks.set_attribute(
                            step, id, 'global_points', points_processed[:, 0])

                        if config.bool('debug'):
                            tracks.set_attribute(step, id, 'global_3D_bbox', get_center_point(
                                config, points_processed[:, 0], tracks.get_detection(step, id)['class']))
                            tracks.set_attribute(
                                step, id, 'global_pointclouds', points[:, 0])
                            tracks.set_attribute(
                                step, id, 'global_points_unprocessed', points[:, 0])
                            tracks.set_attribute(
                                step, id, 'global_colors', colors[:, 0])

                    dynamic_transform, transform_covariance = get_dynamic_transform(
                        points_processed)

                    tracks.set_attribute(
                        step + 1, id, 'dynamic_transforms', dynamic_transform)
                    tracks.set_attribute(
                        step + 1, id, 'transform_covariances', transform_covariance)
                    tracks.set_attribute(
                        step + 1, id, 'global_positions', np.mean(points_processed[:, 1], axis=0))
                    tracks.set_attribute(step + 1,
                                         id,
                                         'position_covariances',
                                         get_position_covariance(points_processed[:,
                                                                                  1],
                                                                 calibration_params))
                    tracks.set_attribute(
                        step + 1, id, 'global_points', points_processed[:, 1])

                    if config.bool('debug'):
                        tracks.set_attribute(step + 1,
                                             id,
                                             'global_3D_bbox',
                                             get_center_point(config,
                                                              points_processed[:,
                                                                               1],
                                                              tracks.get_detection(step,
                                                                                   id)['class'],
                                                              points_processed[:,
                                                                               0]))
                        tracks.set_attribute(step + 1, id, 'global_pointclouds', warp_points(
                            tracks.get_track_attribute(id, 'dynamic_transforms'), id, points_processed[:, 1]))
                        tracks.set_attribute(
                            step + 1, id, 'global_points_unprocessed', points[:, 1])
                        tracks.set_attribute(
                            step + 1, id, 'global_colors', colors[:, 1])
                else:
                    points = np.concatenate((np.expand_dims(point_imgs[step + 1][mask_t1.astype(
                        np.bool)], axis=1), np.expand_dims(point_imgs[step + 1][mask_t1.astype(np.bool)], axis=1)), axis=1)
                    colors = np.concatenate((np.expand_dims(raw_imgs[step + 1][mask_t1.astype(
                        np.bool)], axis=1), np.expand_dims(raw_imgs[step + 1][mask_t1.astype(np.bool)], axis=1)), axis=1)
                    points_processed = remove_outliers(sparsify(points))
                        ...
                        tracks.set_attribute(step + 1,
                                             ...
                                             get_center_point(config,
                                                              ...
                                                              points_processed[:,
                                                                               0]))
                        tracks.set_attribute(
                            step + 1, id, 'global_pointclouds', points[:, 1])
                        tracks.set_attribute(
                            step + 1, id, 'global_points_unprocessed', points[:, 1])
                        tracks.set_attribute(
                            step + 1, id, 'global_colors', colors[:, 1])
                ...
                if not tracks.is_active(step - 1, id):
                    mask_t0 = tracks.get_mask(
                        step, id, postprocess=True).astype(
                        np.bool)
                    points = point_imgs[step][mask_t0]
                    colors = raw_imgs[step][mask_t0]
                    points_processed = remove_outliers(
                        sparsify(
                            np.concatenate(
                                (np.expand_dims(
                                    points, axis=1), np.expand_dims(
                                    points, axis=1)), axis=1)))
                    dynamic_transform = np.asarray([0, 0, 0])
                    transform_covariance = np.eye(3)
                    tracks.set_attribute(
                        step + 1, id, 'dynamic_transforms', dynamic_transform)
                    ...
                    if config.bool('debug'):
                        tracks.set_attribute(step, id, 'global_3D_bbox', get_center_point(
                            config, points_processed[:, 0], tracks.get_detection(step, id)['class']))
                        tracks.set_attribute(
                            step, id, 'global_pointclouds', points)
                        tracks.set_attribute(
                            step, id, 'global_points_unprocessed', points)
