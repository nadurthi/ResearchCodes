...
def visualize_pointcloud(save_path):
    assert os.path.exists(
        save_path + 'all_coords.csv'), 'please compute pointcloud beforehand'

    all_coords = pandas.read_csv(save_path +
                                 'all_coords.csv').to_records().tolist()
    all_coords = np.asarray(all_coords)[:, 1:]
    all_colors = pandas.read_csv(save_path +
                                 'all_colors.csv').to_records().tolist()
    ...
    for step in range(len(raw_imgs)):
        if step % 2 == 0 or step == 0:
            ...
            global_scene_visualization['points'].extend(
                point_img.reshape(shape)[height_mask])
            global_scene_visualization['colors'].extend(
                raw_img.reshape(shape)[height_mask])
        ...
        for ref_id in tracks.get_active_tracks(step):
            ...
            if tracks.get_attribute(
                    step, ref_id, 'global_3D_bbox') is not None:
                global_scene_visualization['points'].extend(
                    get_bbox_points_dense(
                        config, tracks.get_attribute(
                            step, ref_id, 'global_3D_bbox'), tracks.get_detection(
                            step, ref_id)['class']))
                color = np.asarray(colors[ref_id % len(colors)]) * 255
                global_scene_visualization['colors'].extend(
                    np.tile(color, (1208, 1)))

    global_scene_visualization['points'] = np.asarray(
        global_scene_visualization['points'])
    global_scene_visualization['colors'] = np.asarray(
        global_scene_visualization['colors'])
