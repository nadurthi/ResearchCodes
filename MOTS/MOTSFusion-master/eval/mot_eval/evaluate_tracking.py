...
class trackingEvaluation(object):
    ...
    def __init__(
            ...
            t_sha,
            list_seq,
            gt_path="./eval/mot_eval/data/tracking",
            min_overlap=0.5,
            max_truncation=0,
            min_height=25,
            max_occlusion=2,
        ...
        filename_test_mapping = os.path.join(
            gt_path, "evaluate_tracking.seqmap.training")
        self.sequence_name = []
        self.n_frames = []
        ...
        # maximum truncation of an object for evaluation
        self.max_truncation = max_truncation
        # maximum occlusion of an object for evaluation
    ...
    def createEvalDir(self):
        ...
        self.eval_dir = os.path.join(
            "./results/", self.t_sha, "eval", self.cls)
        ...
        for seq, s_name in enumerate(self.sequence_name):
            ...
            if loading_groundtruth:
                ...
                filename = os.path.join(os.path.join(
                    root_dir, s_name), "mot_result")
            f = open(filename, "r")

            # current set has only 1059 entries, sufficient length is checked
            # anyway
            ...
            for line in f:
                ...
                t_data.obs_angle = float(
                    fields[5])          # observation angle [rad]
                ...
                try:
                    ...
                    if id_frame in id_frame_cache and not loading_groundtruth:
                        ...
                        # continue # this allows to evaluate non-unique result
                        # files
                        return False
                ...
                # check if uploaded data provides information for 2D and 3D
                # evaluation
    ...
    def compute3rdPartyMetrics(self, eval_modified):
        ...
        # groundtruth and tracker contain lists for every single frame
        # containing lists of KITTI format detections
        fr, ids = 0, 0
        for seq_idx in range(len(self.groundtruth)):
            ...
            for f in range(len(seq_gt)):
                ...
                # check for ignored FN/TP (truncation or neighboring object
                # class)
                ...
                for gg in g:
                    if gg.tracker < 0:
                        if gg.occlusion > self.max_occlusion or gg.truncation > self.max_truncation or (
                        ...
                        if gg.occlusion > self.max_occlusion or gg.truncation > self.max_truncation or (
                            ...
                            # we want to avoid double counting ignored
                            # detections
                ...
                if tmptp + tmpfp + nignoredtp + \
                        nignoredtracker - nignoredpairs is not len(t):
        ...
        # compute MT/PT/ML, fragments, idswitches for all groundtruth
        # trajectories
        ...
        for seq_idx, (seq_trajectories, seq_ignored) in enumerate(
            ...
            for g, ign_g in zip(
                ...
                last_id = g[0]
                # first detection (necessary to be in gt_trajectories) is
                # always tracked
                ...
                for f in range(1, len(g)):
                    ...
                    if eval_modified:
                        ...
                        # MOTA
                        if last_id != g[f] and last_id != - \
                                1 and g[f] != -1 and g[f - 1] != -1:
                            tmpId_switches += 1
                            self.id_switches += 1
                    ###########################################################
                ...
                # (g[f]!=-1)
                if len(g) > 1 and g[f - 1] != g[f] and last_id != - \
                        1 and g[f] != -1 and not ign_g[f]:
                    tmpFragments += 1
                    self.fragments += 1
        ...
        if self.n_gt != 0:
            if self.id_switches == 0:
                ...
                self.MOTAL = 1 - \
                    (self.fn + self.fp + math.log10(self.id_switches)) / float(self.n_gt)
        else:
    ...
    def createSummary(self):
        ...
        summary += self.printEntry(
            ...
            self.MOTP) + "\n"
        summary += self.printEntry(
            "Multiple Object Tracking Accuracy (MOTAL)",
            self.MOTAL) + "\n"
        summary += self.printEntry(
        ...
        summary += "\n"
        summary += self.printEntry("Ground Truth Objects (Total)",
                                   self.n_gt + self.n_igt) + "\n"
...
# eval modified indicates MOTA* usage
def run_mot_eval(result_path, list_sequences, eval_modified):
