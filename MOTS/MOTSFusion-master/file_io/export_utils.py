...
def export_tracking_result_in_mot_format(
    tracked_sequence,
    out_folder,
    class_names=[
        'Car',
    ...
    with open(out_folder + 'mot_result', "w") as f:
        for step in range(0, tracked_sequence.timesteps):
            for id in tracked_sequence.get_active_tracks(step):
                ...
                print(
                    step, id, label, -1, -1, -10, "{0:.2f}".format(
                        detection['bbox'][0]), "{0:.2f}".format(
                        detection['bbox'][1]), "{0:.2f}".format(
                        detection['bbox'][2]), "{0:.2f}".format(
                        detection['bbox'][3]), -1, -1, -1, -1000, -1000, -1000, -10, file=f)
