...
from pyslam import  slam,kittilocal
import json
import time
import numpy as np
...
D["BinMatch"]={}
D["BinMatch"]["Lmax"]=list(Lmax.astype(np.float64))
D["BinMatch"]["thmax"]=thmax
D["BinMatch"]["thfineres"]=thmin
D["BinMatch"]["dxMatch"]=list(dxMatch.astype(np.float64))
dxBase=np.array([5,5])
D["BinMatch"]["dxBase"]=list(dxBase.astype(np.float64))
...
opt=json.dumps(D)
bm=kittilocal.BinMatch(str(opt))

bm.computeHlevels(X2Dmap_down)
st=time.time()
H12est2=np.identity(3)
sol=bm.getmatch(X1v2D,H12est2)
et=time.time()
