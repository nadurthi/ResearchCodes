...
def non_maximum_suppression(bbox, thresh, score=None,
    ...
    score is only used to sort the boxes
...
def _call_nms_kernel(bbox, thresh):
    ...
    # computes the IOU of each box with everyother box
    # then checks if grester than thres... if so a mask of 1 appears for that combo
    n_bbox = bbox.shape[0]
