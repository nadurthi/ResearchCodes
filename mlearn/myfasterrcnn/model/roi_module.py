...
from pytorchutils.visioncore.roi_cupy import kernel_backward, kernel_forward
...
def load_kernel(kernel_name, code, **kwargs):
    ...
    kernelfunc = cp.RawKernel(code,kernel_name)
    return kernelfunc
...
class RoI(Function):
    ...
    @staticmethod
    def forward(ctx, x, rois,self):
        # NOTE: MAKE SURE input is contiguous too
        # rois is actually indicies and rois, i.e. 5 columns
        x = x.contiguous()
        ...
        ctx.save_for_backward(torch.tensor(self.in_size),self.argmax_data,self.rois,
                              torch.tensor(self.N),torch.tensor(self.spatial_scale)
                              ,torch.tensor(self.outh), torch.tensor(self.outw))
    ...
    def backward(ctx, grad_output):
        ...
        # TODO: input
        in_size, argmax_data, rois,N,spatial_scale,outh,outw = ctx.saved_tensors
        in_size = tuple(in_size.numpy())
        N = int(N.numpy())
        spatial_scale = spatial_scale.numpy()
        outh = int(outh.numpy())
        outw = int(outw.numpy())
...
class RoIPooling2D(t.nn.Module):
    ...
    def forward(self, x, rois):
        return self.RoI.apply(x, rois,self.RoI)
